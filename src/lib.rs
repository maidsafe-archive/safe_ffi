...
use libc::{c_char, c_void, int32_t, size_t, uint8_t, uint64_t};
...
pub extern fn register_dns(client_handle          : *const c_void,
    ...
    let (public_encryption_key, secret_encryption_key) = box_::gen_keypair();
...
mod test {
    ...
    use std::{mem, ptr};
    use libc::{self, c_char, c_void, uint8_t, uint64_t};
    ...
    fn generate_random_cstring(len: usize) -> Result<CString, FfiError> {
        let mut cstring_vec = try!(utility::generate_random_vector::<u8>(len));
    ...
    fn create_directories_files_and_read_files() {
        ...
        let size_of_c_char = mem::size_of::<c_char>();
...

        {
            ...
            unsafe { ptr::copy(cstring_path.as_ptr(), c_path, path_lenght_for_c) };
        }
        ...
        let c_size = unsafe { libc::malloc(size_of_c_uint64) } as *mut uint64_t;
        ...
        {
            let read_cstr_content = unsafe { CStr::from_ptr(c_content as *const c_char) };
...

        {
            ...
            unsafe { ptr::copy(cstring_path.as_ptr(), c_path_blog, path_lenght_for_c) };
        }
        ...
        // --------------------------------------------------------------------
        let c_long_name = unsafe { libc::malloc(SIZE_FOR_C * size_of_c_char) } as *mut c_char;
        ...
        let mut unregistered_client_handle = 0 as *const c_void;
        ...
        // Note: This will result in narrowing on < 64 bit systems - but it's Ok for this test as
        //       we are not dealing with files larger than 2^32 bytes.
        c_content = unsafe { libc::malloc(*c_size as usize * size_of_c_uint8) } as *mut uint8_t;
        ...
        unsafe { libc::free(c_path as *mut c_void) };
        unsafe { libc::free(c_size as *mut c_void) };
        unsafe { libc::free(c_content as *mut c_void) };
        unsafe { libc::free(c_path_blog as *mut c_void) };
        unsafe { libc::free(c_long_name as *mut c_void) };
        unsafe { libc::free(c_file_name_www as *mut c_void) };
        unsafe { libc::free(c_file_name_blog as *mut c_void) };
        unsafe { libc::free(c_service_name_www as *mut c_void) };
        unsafe { libc::free(c_service_name_blog as *mut c_void) };
    }
}
