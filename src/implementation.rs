...
pub fn get_final_subdirectory(client            : Arc<Mutex<Client>>,
                              tokens            : &Vec<String>,
                              starting_directory: Option<&DirectoryKey>) -> Result<DirectoryListing,
                                                                                                                        ::errors::FfiError> {
    ...
    for it in tokens.iter() {
        current_dir_listing = {
            let current_dir_metadata = try!(current_dir_listing.get_sub_directories().iter()
                .find(|a| *a.get_name() == *it).ok_or(FfiError::PathNotFound));
...
pub fn get_file_size(client          : Arc<Mutex<Client>>,
                     name            : &String,
                     parent_directory: &DirectoryListing) -> Result<u64, FfiError> {
...
pub fn get_file_content(client          : Arc<Mutex<Client>>,
                        name            : &String,
                        parent_directory: &DirectoryListing) -> Result<Vec<u8>, FfiError> {
...
fn get_reader<'a>(client          : Arc<Mutex<Client>>,
                  name            : &String,
                  parent_directory: &'a DirectoryListing) -> Result<Reader<'a>, FfiError> {
    let file = try!(parent_directory.get_files().iter()
        .find(|a| *a.get_name() == *name).ok_or(FfiError::FileNotFound));
...
mod test {
    ...
    #[test]
    fn parse_path() {
        let path_0 = unwrap_result!(CString::new("/abc/d/ef").map_err(|error| FfiError::from(error.description())));
        let path_1 = unwrap_result!(CString::new("/abc/d/ef/").map_err(|error| FfiError::from(error.description())));
        let path_2 = unwrap_result!(CString::new("///abc///d/ef////").map_err(|error| FfiError::from(error.description())));
